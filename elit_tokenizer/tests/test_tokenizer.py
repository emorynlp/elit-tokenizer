...
@pytest.mark.parametrize('input, offset, expected', [
    ('  Hello\t\r, world  !\n\n', 0, [Sentence(['Hello', ',', 'world', '!'], [(2, 7), (9, 10), (11, 16), (18, 19)])]),
    ('  Hello\t\r, world  !\n\n', 1, [Sentence(['Hello', ',', 'world', '!'], [(3, 8), (10, 11), (12, 17), (19, 20)])])
])
...
@pytest.mark.parametrize('input, expected', [
    ('', Sentence([], [])),
    (' \t\n\r', Sentence([], [])),
    ('abc', Sentence(['abc'], [(0, 3)])),
    (' A\tBC  D12 \n', Sentence(['A', 'BC', 'D12'], [(1, 2), (3, 5), (7, 10)]))
])
...
@pytest.mark.parametrize('input, offset, expected', [
    ('Hello, world!', 5, Sentence(['Hello', ',', 'world', '!'], [(5, 10), (10, 11), (12, 17), (17, 18)]))
])
...
@pytest.mark.parametrize('input, expected', [
    ...
    (".1 +1 -1 1,000,000.00 1,00 '97 '90s '1990 10:30 1:2 a:b",
     Sentence(['.1', '+1', '-1', '1,000,000.00', '1', ',', '00', "'97", "'90s", "'", '1990', '10:30', '1:2', 'a', ':', 'b'],
    ...
    ("aa;;;b: c\"\"d\"\" 0'''s ''a''a'' 'a'a'a'.?!..",
     Sentence(['aa', ';;;', 'b', ':', 'c', '""', 'd', '""', '0', "'''", 's', "''", 'a', "''", 'a', "''", "'", "a'a'a", "'", '.?!..'],
...
@pytest.mark.parametrize('input, expected', [
    # emoticon
    (':-) A:-( :). B:smile::sad: C:):(! :)., :-))) :---( Hi:).',
     [Sentence([':-)', 'A', ':-(', ':)', '.'], [(0, 3), (4, 5), (5, 8), (9, 11), (11, 12)]),
      Sentence(['B', ':smile:', ':sad:', 'C', ':)', ':(', '!'], [(13, 14), (14, 21), (21, 26), (27, 28), (28, 30), (30, 32), (32, 33)]),
      Sentence([':)', '.'], [(34, 36), (36, 37)]),
      Sentence([',', ':-)))', ':---(', 'Hi', ':)', '.'], [(37, 38), (39, 44), (45, 50), (51, 53), (53, 55), (55, 56)])]),
])
...
@pytest.mark.parametrize('input, expected', [
    ...
    ("'CAUSE 'tis ' em 'happy",
     Sentence(["'CAUSE", "'tis", "'em", "'", 'happy'], [(0, 6), (7, 11), (12, 16), (17, 18), (18, 23)])),
    # acronym
    ('ab&cd AB|CD 1/2 a-1 1-2',
     Sentence(['ab&cd', 'AB|CD', '1/2', 'a-1', '1-2'], [(0, 5), (6, 11), (12, 15), (16, 19), (20, 23)])),
    # hyphenated
    ('mis-predict mic-predict book - able book-es 000-0000 000-000-000 p-u-s-h-1-2',
     Sentence(['mis-predict', 'mic', '-', 'predict', 'book-able', 'book', '-', 'es', '000-0000', '000-000-000', 'p-u-s-h-1-2'],
...
@pytest.mark.parametrize('input, expected', [
    ...
    ('A.B. a.B.c AB.C. Ph.D. 1.2. A-1.',
     [Sentence(['A.B.', 'a.B.c', 'AB.C', '.'], [(0, 4), (5, 10), (11, 15), (15, 16)]),
      Sentence(['Ph.D.', '1.2.', 'A-1.'], [(17, 22), (23, 27), (28, 32)])]),
    # No. 1
    ('No. 1 No. a No.',
     [Sentence(['No.', '1', 'No', '.'], [(0, 3), (4, 5), (6, 8), (8, 9)]),
      Sentence(['a', 'No', '.'], [(10, 11), (12, 14), (14, 15)])])
])
...
@pytest.mark.parametrize('input, expected', [
    ...
    ('20mg 100cm 1st 11a.m. 10PM',
     Sentence(['20', 'mg', '100', 'cm', '1st', '11', 'a.m.', '10', 'PM'],
              [(0, 2), (2, 4), (5, 8), (8, 10), (11, 14), (15, 17), (17, 21), (22, 24), (24, 26)])),
    # concatenated word
    ("whadya DON'CHA",
     Sentence(['wha', 'd', 'ya', 'DO', "N'", 'CHA'], [(0, 3), (3, 4), (4, 6), (7, 9), (9, 11), (11, 14)])),
])
...
@pytest.mark.parametrize('input, expected', [
    ...
    ('Mbaaah.Please hello.!?world',
     [Sentence(['Mbaaah', '.'], [(0, 6), (6, 7)]),
      Sentence(['Please', 'hello', '.!?'], [(7, 13), (14, 19), (19, 22)]),
      Sentence(['world'], [(22, 27)])]),
])
