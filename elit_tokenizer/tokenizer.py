...
class Tokenizer:
    def decode(self, input_text: str, init_offset: int = 0, segment: int = 0) -> Union[Sentence, List[Sentence]]:
        ...
        :param segment: 0 - no segmentation (default), 1 - newline segmentation, 2 - rule-based segmentation, 3 - newline and rule-based segmentation.
        :return: a tokenized sentence if segment == 0; otherwise, a list of tokenized sentences.
        """
        if segment == 0:
        ...
        raise ValueError("Invalid argument: segment = {}".format(segment))

    @abc.abstractmethod
    def tokenize(self, input_text: str, init_offset: int = 0) -> Tuple[List[str], List[Offset]]:
        """
        :param input_text: the input text.
        :param init_offset: the initial offset of the first token.
...
class EnglishTokenizer(Tokenizer):
    def __init__(self, resource_dir: Optional[str] = None):
        ...
        :param resource_dir: a path to the directory containing custom resource files.
        """
        super(EnglishTokenizer, self).__init__()
        resource_root = 'elit_tokenizer.resources'

        def resource_path(name):
            return os.path.join(resource_dir, name) if resource_dir else resource_filename(resource_root, name)

        self.ABBREVIATION_PERIOD = read_word_set(resource_path('english_abbreviation_period.txt'))
        self.APOSTROPHE_FRONT = read_word_set(resource_path('english_apostrophe_front.txt'))
        self.MAP_CONCAT_WORD = read_concat_word_dict(resource_path('english_concat_words.txt'))
        self.HYPHEN_PREFIX = read_word_set(resource_path('english_hyphen_prefix.txt'))
        self.HYPHEN_SUFFIX = read_word_set(resource_path('english_hyphen_suffix.txt'))
